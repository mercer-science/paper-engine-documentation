...
shape, in the right place — a numbered item in the toolkit's own defect list,
with a way to check the fix against. You do not have to know the format.

**Nothing the toolkit records leaves your machine.** There is no upload, no
token to set up and nothing to consent to. Defects are fixed by being told
about them; email **rmercer2@byu.edu** if one is blocking you.
