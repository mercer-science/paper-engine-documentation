...
| `revise-prose` 🔸 | Polishes without changing a single claim, number or citekey — all three are checked unchanged afterwards, and a section that fails is restored. |
...
| `revise-prose --from-comprehension` 🔸 | One bounded rewrite pass whose brief is the comprehension report, over the sections that report named and no others. |
